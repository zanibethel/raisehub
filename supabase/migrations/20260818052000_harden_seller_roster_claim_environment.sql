-- Harden self-service seller roster claims so an authenticated seller cannot
-- mutate a campaign across the RaiseHub Live/Demo boundary.
...
begin
  ...
  -- Campaign-seller rows use canonical organization IDs. Older campaigns may
  -- still rely on organization_id -> profiles.id, so accept that relationship
  -- only when the canonical organization points back to the same legacy profile.
  if not (
    v_campaign.canonical_organization_id = v_organization.id
    or (
      v_campaign.canonical_organization_id is null
      and v_organization.legacy_profile_id = v_campaign.organization_id
    )
  ) then
    raise exception 'Campaign roster organization mismatch.';
  end if;

  -- Fail closed when campaign and organization do not belong to the same
  -- RaiseHub environment. This blocks Demo memberships from mutating Live
  -- campaigns and vice versa, including malformed legacy records.
  if v_campaign.is_demo is distinct from v_organization.is_demo
    ...
    raise exception 'Campaign and organization environment mismatch.';
  end if;
  ...
  if v_membership.is_demo is distinct from v_organization.is_demo
    ...
    raise exception 'Seller membership and organization environment mismatch.';
  end if;
...
revoke all on function public.claim_campaign_seller_roster_entry(uuid, uuid) from public;
revoke all on function public.claim_campaign_seller_roster_entry(uuid, uuid) from anon;
grant execute on function public.claim_campaign_seller_roster_entry(uuid, uuid) to authenticated;
grant execute on function public.claim_campaign_seller_roster_entry(uuid, uuid) to service_role;
