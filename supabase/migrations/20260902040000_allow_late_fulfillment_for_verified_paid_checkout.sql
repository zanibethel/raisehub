-- Allow a verified paid Stripe Checkout Session to fulfill after the local
-- checkout-attempt TTL has elapsed. Signed Stripe webhooks can be delayed by
-- provider retries or delivery outages. Exact session ID, amount, currency,
-- paid state, and existing idempotency checks still apply.
-- Explicitly canceled/expired attempts remain non-fulfillable.
