...
export async function PATCH(request: Request) {
  ...
  if (workspaceRole === 'business') {
    ...
    if (!data?.legacy_profile_id) {
      return NextResponse.json({ error: 'Business profile connection not found.' }, { status: 404 })
    }
    ...
    const { error: businessError } = await admin
      .from('businesses')
      .update({ name: clean(body.name), updated_at: new Date().toISOString() })
      .eq('id', workspaceId)
  ...
  if (workspaceRole === 'organization') {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    if (!data?.legacy_profile_id) {
      return NextResponse.json({ error: 'Organization profile connection not found.' }, { status: 404 })
    }
