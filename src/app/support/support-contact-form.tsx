    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          ...
          <input name="name" required maxLength={120} className="mt-2 min-h-12 w-full rounded-xl border border-slate-200 px-3 text-slate-950 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
        </label>

        <label className="block">
          <span className="text-sm font-bold text-slate-800">Email</span>
          <input name="email" type="email" required maxLength={254} className="mt-2 min-h-12 w-full rounded-xl border border-slate-200 px-3 text-slate-950 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
        </label>
      </div>

      <label className="block">
        <span className="text-sm font-bold text-slate-800">What do you need help with?</span>
        <select name="topic" required value={issueContext.topic} onChange={(event) => setIssueContext((current) => ({ ...current, topic: event.target.value }))} className="mt-2 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-slate-950 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
      ...
      <label className="block">
        ...
        <textarea name="message" required minLength={10} maxLength={4000} rows={6} value={issueContext.details} onChange={(event) => setIssueContext((current) => ({ ...current, details: event.target.value }))} placeholder="Tell us what happened, what you expected, and any steps you already tried." className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3 text-slate-950 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100" />
      </label>

      <button type="submit" disabled={state === 'submitting'} className="inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-blue-700 px-5 text-sm font-black text-white transition hover:bg-blue-800 disabled:cursor-wait disabled:opacity-60 sm:w-auto">
