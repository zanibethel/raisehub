...
export default async function EditOfferPage({ params }: Props) {
  ...
  const { data: offer } = await supabase
    .from('offers')
    .select('id, title, discount, description, starts_at, ends_at, usage_rule, customer_value')
    .eq('id', offerId)
    ...
    <main className="min-h-screen bg-gradient-to-b from-blue-50 via-slate-50 to-white px-4 py-8 sm:px-6 sm:py-12">
      <div className="mx-auto max-w-2xl">
        ...
        <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-8">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">Update the customer-facing details, customer value, redemption frequency, and availability dates for this offer.</p>
