...
export default function BusinessWebsiteBuilderPage() {
  ...
  async function uploadAsset(kind: 'logo' | 'hero', file: File) {
    ...
    setMessage('Image uploaded. Save your site to keep the change.')
    setUploading(null)
  }
  ...
  const renderEditor = (key: string) => {
    ...
    if (key === 'hero') return <div className="mt-5 space-y-4">
      ...
      <label className="block text-sm font-bold">Accent color<input type="color" value={site.accent_color} onChange={(e) => setSite({ ...site, accent_color: e.target.value })} className="mt-2 h-12 w-20 rounded-lg border border-slate-300 bg-white p-1" /></label>
    </div>
  ...
  const renderPreviewSection = (key: string) => {
    if (key === 'hero') return <div key={key} className="relative overflow-hidden p-6 text-white" style={{ backgroundColor: site.accent_color, backgroundImage: site.hero_image_url ? `linear-gradient(rgba(15,23,42,.55),rgba(15,23,42,.55)),url(${site.hero_image_url})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center' }}>{site.logo_url ? <img src={site.logo_url} alt="" className="mb-4 h-14 max-w-40 object-contain" /> : null}<h2 className="text-3xl font-black">{site.hero_heading || business.name}</h2><p className="mt-3 leading-6 opacity-95">{site.hero_copy || 'Tell customers what makes your business worth visiting.'}</p></div>
    if (key === 'about') return <div key={key} className="p-6"><h3 className="text-xl font-black">{site.about_heading}</h3><p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-600">{site.about_copy || 'Add a short description of your business.'}</p></div>
    if (key === 'hours') return site.hours_copy ? <div key={key} className="border-t border-slate-100 p-6"><h3 className="font-black">Hours</h3><p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-600">{site.hours_copy}</p></div> : null
    if (key === 'offers') return site.show_offers ? <div key={key} className="border-t border-slate-100 p-6"><div className="rounded-2xl bg-green-50 p-4"><p className="font-black text-green-800">RaiseHub Offers</p><p className="mt-1 text-sm text-green-700">Active offers will appear here automatically.</p></div></div> : null
    return <div key={key} className="border-t border-slate-100 p-6 text-sm text-slate-600"><h3 className="mb-2 font-black text-slate-900">Contact</h3>{site.phone ? <p>{site.phone}</p> : null}{site.address ? <p>{site.address}</p> : null}{site.contact_email ? <p>{site.contact_email}</p> : null}</div>
  }
    ...
    <section className="mx-auto max-w-5xl">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <div className="space-y-4">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-black">Website address</h2><p className="mt-1 text-sm text-slate-500">Current: raisehub.app/site/{site.slug || 'your-business'} · Subdomain support is being added next.</p><input value={site.slug} onChange={(e) => setSite({ ...site, slug: slugify(e.target.value) })} className="mt-4 w-full rounded-xl border border-slate-300 p-3" placeholder="your-business" /></section>
