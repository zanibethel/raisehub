...
export default function BusinessWebsiteBuilderPage() {
  ...
  return <main className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 px-5 py-8 text-slate-900 sm:px-8">
    <section className="mx-auto max-w-5xl">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">RaiseHub Website + App Builder</p><h1 className="mt-2 text-3xl font-black">Build once for web + app</h1><p className="mt-2 text-slate-600">Create {business.name}'s content once. RaiseHub uses it for the website and installable app.</p></div><div className="flex flex-wrap gap-2"><Link href="/dashboard" className="rounded-xl border border-slate-300 bg-white px-4 py-2 font-bold text-slate-700">Dashboard</Link><Link href={`/site/${site.slug}/admin`} className="rounded-xl border border-blue-300 bg-blue-50 px-4 py-2 font-bold text-blue-800">Business Admin</Link>{site.is_published ? <Link href={publicUrl} target="_blank" className="rounded-xl bg-green-600 px-4 py-2 font-bold text-white">Open Website / App</Link> : null}</div></div>
