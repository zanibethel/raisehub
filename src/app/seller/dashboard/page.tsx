...
export default async function SellerDashboardPage() {
  ...
  let { data: sellerProfile } = await admin
  ...
  const [{ data: campaigns }, { data: organizations }, { data: purchases }] = await Promise.all([
    campaignIds.length
      ? admin
          .from('campaigns')
          .select('id, name, description, goal_amount, starts_at, ends_at, status')
          .in('id', campaignIds)
  ...
  const activeOrganization = activeEntry
    ? organizationsById.get(activeEntry.organization_id) ?? null
    : null
  const activePurchases = activeEntry
    ? purchaseRows.filter((purchase) => purchase.campaign_id === activeEntry.campaign_id)
    : []
  const passesSold = activePurchases.length
  const grossSales = activePurchases.reduce(
    (sum, purchase) => sum + Number(purchase.amount_paid ?? 0),
    0
  )
  const organizationFunds = activePurchases.reduce(
        ...
        {!activeEntry || !activeCampaign ? (
          <section className="rounded-3xl border border-amber-200 bg-white p-6 shadow-lg sm:p-8">
            ...
            <p className="mt-3 max-w-2xl text-gray-600">
              Ask your organizer for the seller signup link. After joining, claim the name they added to the campaign roster so your personal link and sales are connected correctly.
            </p>
            <Link href="/seller/claim-roster" className="mt-6 inline-flex rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white hover:bg-blue-700">
              Claim my roster name
            </Link>
          </section>
        ) : (
          <>
              ...
              <div className="mt-3 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-gray-600">
                    Purchases made through this link are attributed to {activeEntry.display_name} for {activeCampaign.name}.
                  </p>
                  <p className="mt-2 text-sm text-gray-500">
                    {activeOrganization?.name ?? 'Organization'} · Campaign ends {dateLabel(activeCampaign.ends_at)}
                  </p>
                </div>
                <Link href={`/campaigns/${activeCampaign.id}?seller=${encodeURIComponent(activeEntry.referral_code)}`} className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-center text-sm font-bold text-blue-700 hover:bg-blue-100">
                  Preview my campaign page
                </Link>
              </div>
              {shareUrl ? (
                <div className="mt-6 max-w-2xl">
                  <ShareSellerLink url={shareUrl} campaignName={activeCampaign.name} />
                </div>
              ) : null}
            </section>
            ...
            <section className="grid gap-4 sm:grid-cols-3">
              <article className="rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
                <p className="text-sm font-semibold text-gray-500">Passes sold</p>
                <p className="mt-2 text-3xl font-bold text-blue-700">{passesSold}</p>
              </article>
              <article className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-sm">
                <p className="text-sm font-semibold text-gray-500">Sales through your link</p>
                <p className="mt-2 text-3xl font-bold text-emerald-700">{money(grossSales)}</p>
              </article>
              <article className="rounded-2xl border border-violet-100 bg-white p-5 shadow-sm">
                <p className="text-sm font-semibold text-gray-500">Organization funds attributed</p>
                <p className="mt-2 text-3xl font-bold text-violet-700">{money(organizationFunds)}</p>
              </article>
            </section>
                ...
                <div className="mt-4 divide-y divide-slate-100">
                  {activePurchases.slice(0, 8).map((purchase) => (
                    <div key={purchase.id} className="flex items-center justify-between gap-4 py-4">
                      <div>
                        <p className="font-semibold text-gray-900">RaiseHub Pass purchase</p>
                        <p className="mt-1 text-sm text-gray-500">{dateLabel(purchase.created_at)}</p>
                      </div>
              ...
              {entries.map((entry) => {
                ...
                return (
                  <article key={entry.id} className="rounded-2xl border border-slate-200 p-4">
                    ...
                    <Link href={`/campaigns/${entry.campaign_id}?seller=${encodeURIComponent(entry.referral_code)}`} className="mt-3 inline-flex text-sm font-semibold text-blue-700 hover:underline">
                      Open seller link →
                    </Link>
                  </article>
                )
