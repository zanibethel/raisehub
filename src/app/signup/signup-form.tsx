...
function buildCampaignDestination(input: {
  ...
  if (!input.campaignId) return input.source === 'offers' ? '/offers' : '/campaigns'

  const params = new URLSearchParams()
  if (input.seller) params.set('seller', input.seller)
  if (input.donation) params.set('donation', input.donation)
...
export default function SignupForm({ campaigns = [] }: SignupFormProps) {
  ...
  const requestedSeller = searchParams.get('seller')
  const requestedDonation = searchParams.get('donation')
  const requestedOrganization = searchParams.get('organization')

  const initialCampaignId = useMemo(() => {
    if (requestedCampaignId && campaigns.some((campaign) => campaign.id === requestedCampaignId)) return requestedCampaignId
    if (requestedOrganizationId) {
      const matchingCampaign = campaigns.find((campaign) => campaign.organizationId === requestedOrganizationId || campaign.organizationLegacyProfileId === requestedOrganizationId)
  ...
  const organizationCampaigns = useMemo(() => {
    ...
    return campaigns.filter((campaign) => campaign.organizationId === requestedOrganizationId || campaign.organizationLegacyProfileId === requestedOrganizationId)
  }, [campaigns, requestedOrganizationId])

  const endingSoon = useMemo(() => [...campaigns]
    .filter((campaign) => campaign.daysRemaining !== null)
    .sort((left, right) => (left.daysRemaining ?? Infinity) - (right.daysRemaining ?? Infinity))
    .slice(0, 8), [campaigns])

  const almostAtGoal = useMemo(() => [...campaigns]
    .filter((campaign) => campaign.goalPercentage !== null && campaign.goalPercentage < 100)
    .sort((left, right) => (right.goalPercentage ?? -1) - (left.goalPercentage ?? -1))
    .slice(0, 8), [campaigns])

  const campaignGroups = organizationCampaigns.length > 0
    ? [{ title: 'Choose a fundraiser', campaigns: organizationCampaigns }]
    : [
        { title: 'Ending Soon', campaigns: endingSoon },
        { title: 'Almost at the Goal', campaigns: almostAtGoal },
      ]
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          seller_referral: requestedSeller,
        },
      ...
      <div className="rounded-3xl border border-blue-100 bg-white p-6 shadow-xl sm:p-8">
        ...
        {requestedSeller ? (
          <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-900">
            Your seller referral is locked in and will remain attached after signup.
          </div>
        ) : null}

        {selectedCampaign ? (
          <div className="mt-6 rounded-2xl border border-green-200 bg-green-50 p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-green-700">You’re supporting</p>
            <h2 className="mt-2 text-xl font-bold text-gray-900">{selectedCampaign.name}</h2>
            <p className="mt-1 text-sm text-gray-600">{selectedCampaign.organizationName || 'Local organization'}</p>
            <div className="mt-4 flex flex-wrap gap-3 text-sm text-gray-600">
              {selectedCampaign.goalPercentage !== null ? <span>{Math.round(selectedCampaign.goalPercentage)}% of goal</span> : null}
              {selectedCampaign.daysRemaining !== null ? <span>{selectedCampaign.daysRemaining} days remaining</span> : null}
              {selectedCampaign.passPrice !== null ? <span>${selectedCampaign.passPrice.toFixed(2)} pass</span> : null}
            </div>
                  ...
                  {group.campaigns.map((campaign) => (
                    <button key={campaign.id} type="button" onClick={() => setSelectedCampaignId(campaign.id)} className="w-[280px] min-w-[280px] snap-start rounded-2xl border border-blue-100 bg-white p-5 text-left shadow-md transition hover:border-blue-300 hover:shadow-lg">
                      ...
                      <div className="mt-4 space-y-2 text-sm text-gray-600">
                        {campaign.goalPercentage !== null ? <p>{Math.round(campaign.goalPercentage)}% of goal</p> : null}
                        {campaign.amountRemaining !== null ? <p>${campaign.amountRemaining.toLocaleString()} remaining</p> : null}
                        {campaign.daysRemaining !== null ? <p>{campaign.daysRemaining} days remaining</p> : null}
                        {campaign.passPrice !== null ? <p className="font-semibold text-green-700">${campaign.passPrice.toFixed(2)} pass</p> : null}
                      </div>
                      <span className="mt-5 inline-flex rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white">Support This Fundraiser</span>
                    </button>
                  ))}
                </div>
              </section>
