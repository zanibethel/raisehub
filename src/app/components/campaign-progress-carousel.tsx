...
export default function CampaignProgressCarouselClient({
  ...
  const loopCampaigns = useMemo(() => {
    ...
    const count = Math.max(campaigns.length, MINIMUM_LOOP_ITEMS)
  ...
  useEffect(() => {
    ...
    function normalizePosition(currentElement: HTMLDivElement) {
      const loopWidth = currentElement.scrollWidth / 2
  ...
  function handleMouseMove(
    ...
    if (position < EDGE_ZONE_RATIO) {
      const strength =
        (EDGE_ZONE_RATIO - position) / EDGE_ZONE_RATIO
    ...
    if (position > 1 - EDGE_ZONE_RATIO) {
      const strength =
        (position - (1 - EDGE_ZONE_RATIO)) / EDGE_ZONE_RATIO
    ...
    hoverDirectionRef.current = 1
    hoverSpeedRef.current = BASE_SCROLL_PIXELS_PER_SECOND
  }
