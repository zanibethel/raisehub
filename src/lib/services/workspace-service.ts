...
function getBusinessSetupItems(profile: WorkspaceProfile): SetupItem[] {
  return [
    {
      label: 'Business name',
      complete: Boolean(profile.business_name?.trim() || profile.display_name?.trim()),
    },
    ...
    {
      label: 'Description',
      complete: Boolean(profile.business_description?.trim()),
    },
    { label: 'Onboarding', complete: profile.onboarding_completed },
  ]
}

function getOrganizationSetupItems(profile: WorkspaceProfile): SetupItem[] {
  return [
    {
...
function getCustomerSetupItems(profile: WorkspaceProfile): SetupItem[] {
  return [
    {
      label: 'Name',
      complete: Boolean(profile.display_name?.trim() || profile.full_name?.trim()),
    },
    { label: 'Email', complete: Boolean(profile.email?.trim()) },
    { label: 'Phone', complete: Boolean(profile.phone?.trim()) },
  ]
...
function getSetupStatus(setupPercentage: number): string {
...
function getWorkspaceStatus(profile: WorkspaceProfile, setupPercentage: number): string {
  ...
  switch (lifecycleStatus) {
    ...
    case 'inactive':
      return 'Inactive'
    default:
      return getSetupStatus(setupPercentage)
  }
}
