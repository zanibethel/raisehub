      ...
      <div className="border-b border-slate-200 bg-amber-50 p-4 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-amber-950">Support Mode stays read-only until you deliberately enable editing.</p>
          </div>
          <button type="button" onClick={() => { setIsEditing(true); setMessage(null) }} className="rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white hover:bg-slate-800">Edit as Owner</button>
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="border-b border-amber-300 bg-amber-50 p-4 sm:p-6">
      <div className="rounded-xl border border-amber-300 bg-white p-4">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-700">Changes save directly to this client profile. Review carefully before saving.</p>
      </div>
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-semibold text-slate-700 sm:col-span-2">Phone<input name="phone" defaultValue={workspace.phone ?? ''} className="mt-2 w-full rounded-xl border border-slate-300 px-3 py-2.5" /></label>
      </div>
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        <button type="button" onClick={() => { setIsEditing(false); setMessage(null) }} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold text-slate-700">Cancel</button>
      </div>
