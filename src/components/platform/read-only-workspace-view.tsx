  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid min-w-0 gap-4 border-b border-slate-200 p-4 sm:p-6 lg:grid-cols-3">
        ...
        <article className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-2 space-y-1.5">
            <p className="break-all text-sm font-semibold text-slate-900">{workspace.email ?? 'No email added'}</p>
            <p className="break-words text-sm text-slate-600">{workspace.phone ?? 'No phone added'}</p>
          </div>
        </article>
      </div>
        ...
        {missingItems.length > 0 ? (
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {missingItems.map((item) => (
              <div key={item} className="flex min-w-0 items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
                <span className="h-2 w-2 shrink-0 rounded-full bg-amber-500" aria-hidden="true" />
                <span className="min-w-0 break-words text-sm font-semibold text-amber-950">{item} needed</span>
              </div>
            ))}
          </div>
        ) : (
          <div className="mt-4 rounded-xl border border-green-200 bg-green-50 p-4">
            <p className="text-sm font-semibold text-green-800">All current profile setup requirements are complete.</p>
          </div>
        )}
      </div>
      ...
      {workspace.role === 'customer' ? (
        <ReadOnlyCustomerActivitySection activityResult={customerActivityResult} />
      ) : null}
    </section>
  )
}
