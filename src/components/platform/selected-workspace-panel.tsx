    ...
    <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 bg-slate-50 px-4 py-4 sm:px-6">
        <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-slate-600">{getRoleLabel(workspace.role)}{workspace.planLabel ? ` · ${workspace.planLabel}` : ''}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full border border-slate-300 bg-white px-3 py-1 text-xs font-bold text-slate-700">{getStatusLabel(workspace.status)}</span>
            <span className={`rounded-full border px-3 py-1 text-xs font-bold ${isReadOnly ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-blue-200 bg-blue-50 text-blue-700'}`}>{getModeLabel(mode)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid min-w-0 gap-4 p-4 sm:p-6 md:grid-cols-3">
        ...
        <article className="rounded-xl border border-slate-200 p-4">
          ...
          <p className="mt-1 text-sm leading-5 text-slate-600">{isArchived || isRestoreRequested ? 'The supporter account remains active while this workspace is hidden.' : 'This workspace may appear in production discovery.'}</p>
        </article>
      </div>
      ...
      <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-4 py-4 sm:px-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            {!isReadOnly ? <Link href={supportUrl} className="inline-flex min-h-10 items-center justify-center rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-900">Enter Support Mode</Link> : null}
            <Link href="/dashboard" className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700">Close Workspace</Link>
          </div>
        </div>
        ...
        {isBusiness && !isReadOnly ? (
          <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {isArchived || isRestoreRequested ? (
              <button type="button" onClick={() => updateBusinessLifecycle('restore')} disabled={isUpdating} className="rounded-lg bg-green-700 px-3 py-2 text-sm font-bold text-white disabled:opacity-60">{isUpdating ? 'Updating…' : 'Restore Workspace'}</button>
            ) : (
              <button type="button" onClick={() => updateBusinessLifecycle('archive')} disabled={isUpdating} className="rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-sm font-bold text-red-800 disabled:opacity-60">{isUpdating ? 'Updating…' : 'Archive Workspace'}</button>
            )}
          </div>
        ) : null}
      </div>
