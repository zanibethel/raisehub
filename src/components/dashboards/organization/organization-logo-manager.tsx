...
export default function OrganizationLogoManager({ organizationId }: Props) {
  ...
  useEffect(() => {
    let active = true
    fetch(`/api/organization/logo?organizationId=${encodeURIComponent(organizationId)}`)
      .then(async (response) => {
        const result = (await response.json().catch(() => ({}))) as { logoUrl?: string | null }
        if (active && response.ok) setLogoUrl(result.logoUrl ?? '')
      })
      .catch(() => undefined)
    return () => { active = false }
  }, [organizationId])
  ...
  return (
    <details className="group rounded-2xl border border-blue-100 bg-white/90 shadow-xl backdrop-blur">
      ...
      <div className="space-y-4 border-t border-blue-100 p-5 sm:p-6">
        ...
        <input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={(event) => setFile(event.target.files?.[0] ?? null)} className="block w-full rounded-xl border border-gray-300 bg-white p-3 text-sm" />
        <div className="flex flex-col gap-3 sm:flex-row">
          <button type="button" onClick={uploadLogo} disabled={loading || !file} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50">{loading ? 'Saving…' : logoUrl ? 'Replace logo' : 'Upload logo'}</button>
          {logoUrl ? <button type="button" onClick={removeLogo} disabled={loading} className="rounded-xl border border-red-200 bg-red-50 px-5 py-3 text-sm font-bold text-red-700 hover:bg-red-100 disabled:opacity-50">Remove logo</button> : null}
        </div>
