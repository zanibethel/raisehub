...
const WorkspaceStatusContext =
  createContext<WorkspaceStatusContextValue | null>(null)
const ignoreStandaloneStatus = (
  _item: WorkspaceItem,
...
function StatusItem({
  ...
  const symbolClassName = isComplete
    ? 'text-green-600'
    : isChecking
      ? 'text-gray-400'
      : 'text-amber-600'
...
export function WorkspaceStatusReporter({
  item,
  status,
}: {
  item: WorkspaceItem
...
export default function OrganizationWorkspaceStatus({
  children,
}: {
  children: React.ReactNode
}) {
  const [statuses, setStatuses] = useState<
    Record<WorkspaceItem, WorkspaceStatus>
  >({ profile: 'checking', payouts: 'checking' })
  const reportStatus = useCallback(
    (item: WorkspaceItem, status: WorkspaceStatus) => {
      setStatuses((current) =>
        current[item] === status ? current : { ...current, [item]: status }
      )
    },
    []
  )
  const value = useMemo(() => ({ reportStatus }), [reportStatus])
  const needsAttention = Object.values(statuses).some(
    (status) => status === 'attention'
  )
  const profileNeedsAttention = statuses.profile === 'attention'

  return (
    <WorkspaceStatusContext.Provider value={value}>
      <details
        className={`group rounded-2xl border bg-white/90 shadow-sm backdrop-blur ${
          needsAttention ? 'border-amber-200' : 'border-blue-100'
        }`}
      >
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 sm:px-5">
          <div className="min-w-0">
            <p
              className={`text-xs font-bold uppercase tracking-wide ${
                needsAttention ? 'text-amber-700' : 'text-blue-700'
              }`}
            >
              Workspace status
            </p>
          ...
          <span
            className={`shrink-0 rounded-full px-3 py-2 text-sm font-semibold group-open:hidden ${
              needsAttention
                ? 'bg-amber-50 text-amber-700'
                : 'bg-blue-50 text-blue-700'
            }`}
          >
            {needsAttention ? 'Review' : 'Manage'}
          </span>
        ...
        <div className="divide-y divide-blue-100 border-t border-blue-100 px-4 sm:px-5">
            ...
            <div className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Add the required organization details before publishing a
                  campaign.
                </p>
              </div>
              <Link
                href="/dashboard/organization/profile"
                className="inline-flex shrink-0 items-center justify-center rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-bold text-white hover:bg-amber-600"
              >
                Edit profile
              </Link>
            </div>
