...
type WorkspaceStatus = 'checking' | 'complete' | 'attention'
...
export function useWorkspaceStatusReporter() {
  ...
  if (!context) {
    throw new Error('useWorkspaceStatusReporter must be used within OrganizationWorkspaceStatus')
  }
...
export default function OrganizationWorkspaceStatus({ children }: { children: React.ReactNode }) {
  const [statuses, setStatuses] = useState<Record<WorkspaceItem, WorkspaceStatus>>({
    profile: 'checking',
    payouts: 'checking',
  })
      ...
      >
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 sm:px-5">
          <div className="min-w-0">
            <p className={`text-xs font-bold uppercase tracking-wide ${needsAttention ? 'text-amber-700' : 'text-blue-700'}`}>
              Workspace status
            </p>
          ...
          <span className={`shrink-0 rounded-full px-3 py-2 text-sm font-semibold group-open:hidden ${
            needsAttention ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'
          }`}>
