...
export default function OrganizationTopSellersSection({
  ...
  function downloadCsv() {
    ...
    const rows = [
      ...
      ['organization_name', reportValue(organization?.organizationName)],
      ['organization_type', getOrganizationTypeLabel(organization?.organizationType)],
      ['legal_entity_name', reportValue(organization?.legalEntityName)],
      ['tax_id_masked', organization ? maskTaxIdLast4(organization.taxIdLast4) : 'Not provided'],
      ['tax_exempt_status', getTaxExemptStatusLabel(organization?.taxExemptStatus)],
      ['authorization_status', getAuthorizationStatusLabel(organization?.authorizationStatus)],
      ['authorization_contact_name', reportValue(organization?.authorizationContactName)],
      ['authorization_contact_role', reportValue(organization?.authorizationContactRole)],
      ['authorization_contact_email', reportValue(organization?.authorizationContactEmail)],
      ['organization_data_source', organization?.source ?? 'Not provided'],
      ['organization_data_captured_at', reportValue(organization?.capturedAt)],
      [],
