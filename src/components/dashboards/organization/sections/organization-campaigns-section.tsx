...
function reviewLabel(status?: string | null) {
  switch (status) {
    ...
    case 'suspended': return 'Review suspended'
    default: return 'Review not submitted'
  }
}
...
function ReviewBadge({ status }: { status?: string | null }) {
  ...
  return (
    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${approved ? 'bg-green-100 text-green-800' : pending ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700'}`}>
      {reviewLabel(status)}
    </span>
  )
}
